...
public class TGPGameAPI implements BaseInterface {
	...
	private String API_URL = "http://staging.tgpaccess.com";
//	private String brand_code = "dfh";
	private String client_id = "dfh";
	private String client_secret = "EO0jMfCFFixeBajuiFz1KsEgHTqQqsFJWF7aN3Z1iUfe";
	private String istestplayer = "true";
	
//	Lobby URL – desktop	http://staging.tgpasia.com/E88T5D?token={authtoken}
//	Lobby URL – mobile	http://staging.tgpasia.com/E88T5M?token={authtoken}
	
	private String LobbyDomain = "http://staging.tgpasia.com";
...
//			http://<SGSIntegrationAPIServer>/player/balance?userid=fa39953d&cur=RMB
	...
	public Object deposit(Map<String, Object> entity) {
		...
		if(MapUtil.isNulls(entity, "username,orderid,amount")){
			...
			jsonObject.put("txid", entity.get("orderid"));
			jsonObject.put("timestamp", SGSUtil.getIsoDateCurren());
			System.out.println("submit param："+jsonObject.toString());
        ...
        try {
            ...
            if(httpResponse.getStatusLine().getStatusCode() == 200)  {
    			...
    			if(jsonObject.has("access_token")) {
    				System.out.println("成功获取到 TOKEN="+jsonObject.getString("access_token"));
...
//			5  Diamond - VVIP limits
			...
			JSONObject object_request = new JSONObject();
			object_request.put("ipaddress", "103.5.4.246");
			object_request.put("username", entity.get("username"));
			object_request.put("userid", entity.get("username"));
			object_request.put("lang", "zh-CN");
			object_request.put("cur", "RMB");
			...
			System.out.println("请求参数："+object_request.toString());
			...
			if( !object.has("code") ) {
				...
				if(deviceType.equals("0")) {//PC
					...
					if( playtype.equals("SUNBET_SX") ) {
						...
						return Enum_MSG.成功.message(LobbyDomain+"/E88T5D?token="+authtoken);//token
...
//		password=XQ5Lv5Xq, loginname=zOne6KTd83sMEpb
		...
		//{"authtoken":"3tXJipPZEDrdjUxF0mD7adm5eXXb3FLVUE8ZFrRTtF07C9k8Tm9gZXJPZXvO6gxY1NTGlREXhTzEzhxWXzRbnOtyDJLF6i1AFlgoMWc0paCzuCz4odIPFLVNcE4IF3Hw0Qi","isnew":true}
//		System.out.println(tag.createAccount(entity));//username,password
		
//		
		entity.put("orderid", "IN"+RandomString.createRandomString(18));
		entity.put("amount", "2");
//		{"bal":4.00,"cur":"RMB","txid":"IN5maQ0TXEsQ3snuee40","ptxid":"3e0152d5-f3f4-e711-80c0-0050568c10c1","dup":false}
