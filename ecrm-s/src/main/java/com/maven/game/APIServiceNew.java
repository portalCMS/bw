...
public class APIServiceNew {
	...
	public String play(String gametype,String playtype,String employeecode,List<Game> listGame,String devicetype,String gamecode, Map<String, String> urlMap) throws Exception{
		...
		EmployeeApiAccout ea = SystemCache.getInstance().getEmployeeGameAccount(employeecode, gameTypeTemp);
		System.out.println("gameTypeTemp="+gameTypeTemp+" EmployeeApiAccout="+(ea==null));
		if(ea ==null) {
		...
		if(!gametype.equals(Enum_GameType.NB体育.gametype)){
			if(enterprise != null && enterprise.getTransfertype().equals(Enterprise.Enum_transfertype.自动转分.value)) {
				this.transfer(gameTypeTemp,employeecode,listGame);
			}
		}
		String password =ea.getGamepassword();
		if(gametype.equals(Enum_GameType.YSB体育.gametype)){
			password =ea.getEmployeecode();
		}
		
		return GameAPI.login(ea.getGameaccount(), password, gametype, playtype, ea.getEnterprisecode(), devicetype, gamecode, employeecode, null, urlMap).toString();
	}
	...
	public Map<String, String> balancesAll(String employeecode,String brandcode) throws Exception{
		...
		if(eapiaccounts!=null){
			for (EmployeeApiAccout e : eapiaccounts.values()) {
				...
				//捕鱼游戏共用AG\SA余额
				if (e.getGametype().equals(Enum_GameType.Bet67AG捕鱼.gametype) || e.getGametype().equals(Enum_GameType.Bet67SA捕鱼.gametype)) {
					continue;
				}
				try {
					JSONObject object = JSONObject.fromObject( GameAPI.getBalance(e.getGameaccount(), e.getGamepassword(), e.getGametype(), e.getEnterprisecode()) );
	 ...
	 * @throws Exception 
	 */
	public String balacneCenter(String employeecode) throws Exception {
		EnterpriseEmployeeCapitalAccount ec = capitalAccountService.takeCurrencyAccount(employeecode);
		if(ec == null) return Enum_MSG.资金账户不存在.desc;
		BigDecimal centerBalance = ec.getBalance();
		return Enum_MSG.成功.message("0",centerBalance.setScale(2, RoundingMode.HALF_UP).toString());
	}
	/**
	 * 用户下分
	 * @param employeecode
...
//		
//		//调用service层的东西
//		if(true) return moneyInAndOutService.upIntegral(beforeAmount, orderamount, gametype, ea, patchno);
//		
//		/*********获取上分的订单号**********/
...
//	private static Map<String,String> mapGamePlatformPrefix = null;
	...
	private  String create(String gametype,EnterpriseEmployee ee, String username, String password, String suffix) throws Exception{
		...
		if(GameEnum.Enum_GameType.AV老虎机.gametype.equals(gametype) ) {//AV必须要加前缀，也就是有12位数是可以使用的
			...
			max = 15;
			step = 7;
		} else if(GameEnum.Enum_GameType.PT.gametype.equals(gametype) || GameEnum.Enum_GameType.WIN88.gametype.equals(gametype) || GameEnum.Enum_GameType.IMPT.gametype.equals(gametype) ) {
			max = 20;
			step = 8;
		...
		if(!GameEnum.Enum_GameType.YSB体育.gametype.equals(gametype)){
		...
		*/
		...
		if(object.getString("code").equals("0")){
			...
			if(GameEnum.Enum_GameType.波音.gametype.equals(gametype) || GameEnum.Enum_GameType.沙龙.gametype.equals(gametype) || GameEnum.Enum_GameType.洲际.gametype.equals(gametype) || GameEnum.Enum_GameType.IDN.gametype.equals(gametype) || GameEnum.Enum_GameType.M88.gametype.equals(gametype) || GameEnum.Enum_GameType.GG.gametype.equals(gametype) ) {
				...
				try {
					...
					TLogger.getLogger().Error("==========无更多可用后缀，出现错误==========", e);
					e.printStackTrace();
