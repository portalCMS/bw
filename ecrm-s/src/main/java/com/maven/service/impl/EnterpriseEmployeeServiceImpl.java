...
public class EnterpriseEmployeeServiceImpl extends BaseServiceImpl<EnterpriseEmployee>
	...
	public List<EnterpriseEmployee> takeDerictly(Map<String, Object> object) throws Exception {
		return enterpriseEmployeeDao
				.takeDerictly(AttrCheckout.checkout(object, false, new String[] { "selfecode", "superiorecode" }));
	}

	@Override
	public int takeDerictlyCount(Map<String, Object> object) throws Exception {
		return enterpriseEmployeeDao
				.takeDerictlyCount(AttrCheckout.checkout(object, false, new String[] { "selfecode", "superiorecode" }));
	}
	 ...
	 */
	@Override
	public void tc_saveUser(EnterpriseEmployee enterpriseEmployee, List<EmployeeGamecataloyBonus> bonus,
			boolean isDirectly) throws Exception {
		// 保存用户
		enterpriseEmployee = AttrCheckout.checkout(enterpriseEmployee, false,
				new String[] { "enterprisecode", "parentemployeecode", "parentemployeeaccount", "employeetypecode",
						"loginaccount", "loginpassword", "fundpassword", "displayalias", "employeestatus",
						"onlinestatus", "dividend", "share" });
		List<EnterpriseEmployee> __exit = takeByLoginAccount(enterpriseEmployee.getLoginaccount());
		if (__exit.size() > 0)
			throw new LogicTransactionRollBackException("用户名已存在");
		enterpriseEmployeeDao.saveEnterpriseEmployee(enterpriseEmployee);

		// 创建用户的资金帐户
		enterpriseEmployeeCapitalAccountService.tc_saveEmployeeCapitalAccount(new EnterpriseEmployeeCapitalAccount(
				enterpriseEmployee.getEmployeecode(), enterpriseEmployee.getParentemployeecode()));
		...
		// 对代理用户进行授权
		if (!isDirectly && !enterpriseEmployee.getEmployeetypecode().equals(EnterpriseEmployeeType.Type.会员.value)) {
			// employeeMappingMenuService.tc_agentAuthorization(enterpriseEmployee);//暂时不设置默认权限
		}
		/****
		 * //创建元宝账户 if(enterpriseEmployee.getEmployeetypecode().equals(
		 * EnterpriseEmployeeType.Type.代理.value) ||
		 * enterpriseEmployee.getEmployeetypecode().equals(
		 * EnterpriseEmployeeType.Type.信用代理.value) ) {
		 * baccarath5BalanceService.addActivityBetRecord(new Baccarath5Balance(
		 * enterpriseEmployee.getEmployeecode(),
		 * enterpriseEmployee.getLoginaccount(),
		 * enterpriseEmployee.getEnterprisecode(),
		 * enterpriseEmployee.getBrandcode(),
	...
	public void tc_saveEnterprise(EnterpriseEmployee enterpriseEmployee) throws Exception {
		AttrCheckout.checkout(enterpriseEmployee, false, new String[] { "enterprisecode", "loginaccount",
				"loginpassword", "parentemployeecode", "parentemployeeaccount" });
		// 保存员工
		List<EnterpriseEmployee> __exit = takeByLoginAccount(enterpriseEmployee.getLoginaccount());
		if (__exit.size() > 0)
			throw new LogicTransactionRollBackException("用户名已存在");
		enterpriseEmployee.setEmployeetypecode(EnterpriseEmployeeType.Type.业主.value);
		...
		// 设置企业号洗码值
		employeeGamecataloyBonusService.tc_settingEnterpriseBonus(new EmployeeGamecataloyBonus(
				enterpriseEmployee.getEmployeecode(), enterpriseEmployee.getParentemployeecode()));
	...
	@Override
	public int queryLoginNamePhoneExists(String object) throws Exception {
		return enterpriseEmployeeDao.queryLoginNamePhoneExists(object);
	}
	...
	public int updateOnlineStatus(EnterpriseEmployee ee) throws Exception {
		return enterpriseEmployeeDao
				.updateOnlineStatus(AttrCheckout.checkout(ee, false, new String[] { "employeecode", "onlinestatus" }));
	}
	...
	public EnterpriseEmployee takeEmployeeByCode(String employeecode) throws Exception {
		if (employeecode == null)
			throw new ArgumentValidationException("parameter [ employeecode ] is null");
	...
	public int tc_setDividendShare(EnterpriseEmployee ee) throws Exception {
		return enterpriseEmployeeDao.setDividendShare(
				AttrCheckout.checkout(ee, true, new String[] { "employeecode", "dividend", "share" }));
	}
	 ...
	 */
	@Override
	public List<EnterpriseEmployee> call_uspActivityButmoneyEnterprisecodeBigType(Map<String, Object> paramObj)
			throws Exception {
	 ...
	 */
	@Override
	public List<EnterpriseEmployee> call_uspActivityButmoneyEnterprisecodeSmallType(Map<String, Object> paramObj)
			throws Exception {
	...
	@Override
	public EnterpriseEmployee selectEmployeeByEnterpriseCode(String enterprisecode) throws Exception {
		return enterpriseEmployeeDao.selectEmployeeByEnterpriseCode(enterprisecode);
	}
