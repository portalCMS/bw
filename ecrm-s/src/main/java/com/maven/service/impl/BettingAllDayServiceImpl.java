...
public class BettingAllDayServiceImpl extends BaseServiceImpl<BettingAllDay> implements BettingAllDayService{
	...
	public void updateDoPlan(String planDate,String newPatchNo) throws Exception {
		...
		List<BettingAllGameWinloseDetail> list = bettingAllGameWinloseDetailDao.selectGroup(paramObj);
		...
		List<BettingAllDay> listAdd = new ArrayList<BettingAllDay>();
		for (BettingAllGameWinloseDetail data : list) {
			...
			if(data.getBetmoney() == 0 || data.getNetmoney() == 0 || data.getValidbet() == 0) {
				...
				userName = ee.getLoginaccount();
				
				listAdd.add(new BettingAllDay( enterprisecode,   brandcode, 
		...
		if(listAdd.size() > 0) {
			BettingAllDayDao.saveRecordBatch(listAdd);
		}
		System.out.println("updateDoPlan=生成汇总记录数"+listAdd.size());
	 ...
	 */
	@Override
	public void updateDoPlanByHand(String planDate,String newPatchNo) throws Exception {
		
		/*******************手动计算利润报表1= 先汇总所有明细数据到汇总表*******************/
		
		Map<String, Object> paramObj = new HashMap<String, Object>();
		paramObj.clear();
		paramObj.put("bettime", planDate);
		paramObj.put("patchno", null);
	    bettingAllGameWinloseDetailDao.updateByPatchnoToNull(paramObj);
		...
		if(__listAdd.size() > 0) {
			BettingAllDayDao.deleteRecordBatch(planDate);
			BettingAllDayDao.saveRecordBatch(__listAdd);
			BettingAllDayDao.updateStatusAllDayByBetDate(planDate);
		}
