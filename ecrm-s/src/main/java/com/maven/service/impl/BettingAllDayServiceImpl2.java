...
public class BettingAllDayServiceImpl2 extends BaseServiceImpl<BettingAllDay2> implements BettingAllDayService2{
	...
	private static List<String> ALL_AGENTTYPE = new ArrayList<String>(){{
		...
		this.add(EnterpriseEmployeeType.Type.大股东.value);
		this.add(EnterpriseEmployeeType.Type.股东.value);
		this.add(EnterpriseEmployeeType.Type.总代理.value);
	...
	public int updateDoPlan(String planDate,String newPatchNo) throws Exception {
		...
		Map<String, BettingAllDay2> calcUser = new HashMap<String, BettingAllDay2>();
		
		
		Date addTime = new Date();
		EnterpriseEmployee ee = null;
		List<BettingAllDay2> listData = new ArrayList<BettingAllDay2>();
			...
			if(data.getBetmoney() == 0 && data.getNetmoney() == 0 && data.getValidbet() == 0) {
				continue;
					...
					if(mapUser.containsKey(employeecode)) {
						ee = mapUser.get(employeecode);
					} else {
						ee = enterpriseEmployeeService.takeEmployeeByCode(employeecode);
					}
					enterprisecode = ee.getEnterprisecode();
					brandcode = ee.getBrandcode();
					parentemployeecode = ee.getParentemployeecode();
					if("EN0044".equals(ee.getEnterprisecode())){
						if(calcUser.containsKey(employeecode+"_"+data.getGamebigtype())){
							BettingAllDay2 temp =calcUser.get(employeecode+"_"+data.getGamebigtype());
							temp.setValidMoney(temp.getValidMoney()+data.getValidbet());
							temp.setBetMoney(temp.getBetMoney()+data.getBetmoney());
							temp.setNetMoney(temp.getNetMoney()+data.getNetmoney());
							calcUser.put(employeecode, temp);
						}else{
							BettingAllDay2 temp = new BettingAllDay2(enterprisecode, brandcode, employeecode, parentemployeecode, 
									ee.getLoginaccount(), data.getGamebigtype(),data.getGamebigtype(), null, Integer.valueOf(planDate), 
									data.getBetmoney(),data.getNetmoney(), data.getValidbet(), addTime, newPatchNo);
							calcUser.put(employeecode, temp);
						}
					}else{
						listData.add(new BettingAllDay2(enterprisecode, brandcode, employeecode, parentemployeecode, 
							ee.getLoginaccount(), data.getGametype(), data.getGamebigtype(), null, Integer.valueOf(planDate), 
							data.getBetmoney(),data.getNetmoney(), data.getValidbet(), addTime, newPatchNo));
					}
				
			}
		}
		calcUser.forEach((k,v)->{
			listData.add(v);
		});
		if(listData.size() > 0) {
			BettingAllDayDao.saveRecordBatch(listData);
		}
		System.out.println("updateDoPlan=生成汇总记录数"+listData.size());
