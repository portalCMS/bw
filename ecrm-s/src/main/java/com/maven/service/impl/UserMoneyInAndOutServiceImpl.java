...
public class UserMoneyInAndOutServiceImpl extends BaseServiceImpl<UserMoneyInAndOut>
	...
	public void transfer(String gametype,String employeecode,List<Game> listGame1) throws Exception {
		...
		TLogger.getLogger().Info("用户:"+employeecode+" 开始进行游戏转分...批次号："+patchno);
...
//			List<String> brand_open_game = brandOpenGame(games);
		...
		if(eapiaccounts!=null){
			...
			if( !APIServiceUtil.mapLastLoginGame.containsKey(employeecode)) {
				UserMoneyInAndOut temp = accountChangeRecordDao.findMaxUpRecord(employeecode);
				if(temp != null) {
			...
			if( APIServiceUtil.mapLastLoginGame.containsKey(employeecode) ) {
				...
				EmployeeApiAccout e = eapiaccounts.get(gametypeLast);
				try {
					//获取游戏账户余额
					...
					if(balance.intValue()>1){
						TLogger.getLogger().Info("用户:"+employeecode+" 正在进行对游戏"+e.getGametype()+" 进行游戏下分,金额："+balance.intValue()+"...批次号："+patchno);
					...
					e2.printStackTrace();
					TLogger.getLogger().Info("用户:"+employeecode+" "+e.getGametype()+" 游戏转分失败"+"...批次号："+patchno,e2);
				}
			}else {
...
//					//本次上分目标与最后一次相同时，则不下分
//					if(e.getGametype().equals(gametype)) {
//						TLogger.getLogger().Info("用户:"+employeecode+" "+e.getGametype()+"最后一次上分游戏是"+gametype+"，本次上分目标是："+gametype+"，游戏类型相同，本次转分不进行下分，直接上分...批次号："+patchno);
//						continue;
//					}
//					
//					//特别处理东海国际
...
//					
//				} else {
//					TLogger.getLogger().Info("用户:"+employeecode+" "+e.getGametype()+"其他游戏跳过不下分...批次号："+patchno);
//				}
//			}
//				
//			/*****************************************************新下分逻辑 （对最后一次游戏下分） ***************************************************/
//			
		...
		if(accountfund.intValue()>1){
			...
			TLogger.getLogger().Info("用户:"+employeecode+" 正在对游戏"+ea.getGametype()+" 进行游戏上分,当前资金余额："+accountfund.intValue()+"...批次号："+patchno);
			
			upIntegral(beforeAmount,accountfund.intValue(),gametype,ea, patchno);
			
			
		} else {
			TLogger.getLogger().Info("用户:"+employeecode+" 正在对游戏"+gametype        +" 进行游戏上分,当前资金余额："+accountfund.intValue()+"，但当前此人账户余额只有"+accountfund.doubleValue()+"，所以不进行上分...批次号："+patchno);
		}
		SystemCache.getInstance().setEmployeeAllGameAccount(employeecode, eapiaccounts);
		TLogger.getLogger().Info("用户:"+employeecode+" 游戏转分结束..."+"...批次号："+patchno);
	}
	...
	public boolean upIntegral(BigDecimal beforeAmount,int orderamount,String gametype ,EmployeeApiAccout ea, String patchno) throws Exception{
		...
		TLogger.getLogger().Info("用户:"+ea.getEmployeecode()+" 准备上分..批次号："+patchno);
		...
		TLogger.getLogger().Info("用户:"+ea.getEmployeecode()+" 扣除用户账户资金,金额："+orderamount+"...批次号："+patchno);
		...
		String gamePassword = ea.getGamepassword();
		if(gametype.equals(GameEnum.Enum_GameType.YSB体育.gametype)){
			gamePassword = ea.getEmployeecode(); // 充当UID
		}
		
		//API接口上分
		JSONObject object = JSONObject.fromObject( GameAPI.deposit(ea.getGameaccount(), gamePassword, gametype, ea.getEnterprisecode(), ordernumber, orderamount) );
		
		TLogger.getLogger().Info("用户:"+ea.getEmployeecode()+"  " +gametype+" 游戏上分结果:"+object+"...批次号："+patchno);
		...
		if(object.getString("code").equals("0")){
			...
			if(object__balance.getString("code").equals("0")){
				...
				TLogger.getLogger().Info("用户:"+ea.getEmployeecode()+"  " +gametype+" 更新状态成功.count="+count+",moneyinoutcode="+umiao.getMoneyinoutcode()+"。当前余额："+__balancecurren+"，上分前余额:"+beforeAmount+"，本次上分金额："+orderamount+"...批次号："+umiao.getPatchno()+"");
				...
				TLogger.getLogger().Info("用户:"+ea.getEmployeecode()+"  " +gametype+" 更新状态成功.count="+count+",moneyinoutcode="+umiao.getMoneyinoutcode()+"。但二次查询余额出错："+object__balance+"，上分前余额:"+beforeAmount+"，本次上分金额："+orderamount+"...批次号："+umiao.getPatchno());
			}
		...
		//失败
			...
			if( !gametype.equals(GameEnum.Enum_GameType.MG.gametype) && !gametype.equals(GameEnum.Enum_GameType.棋牌.gametype)) {//只有MG不支持订单查询
				...
				TLogger.getLogger().Info("查询订单状态返回："+object_order);
				if(object_order.getString("code").equals("0")){
					...
					TLogger.getLogger().Info("用户:"+ea.getEmployeecode()+"  " +gametype+" 更新状态失败。当前余额："+beforeAmount+"，上分前余额:"+beforeAmount+"，本次上分金额："+orderamount+"...批次号："+umiao.getPatchno());
...
//					return false;
					throw new Exception(Enum_MSG.系统错误.desc);
				}
...
//				return false;
	...
	public boolean downIntegral(BigDecimal beforeAmount,int downIntegralAmount,String gametype ,EmployeeApiAccout eaa, String patchno) throws Exception{
		...
		String gamePassword = eaa.getGamepassword();
		if(gametype.equals(GameEnum.Enum_GameType.YSB体育.gametype)){
			gamePassword = eaa.getEmployeecode(); // 充当UID
		}
		
		//进行下分操作
		JSONObject object = JSONObject.fromObject( GameAPI.withdraw(eaa.getGameaccount(), gamePassword, gametype, eaa.getEnterprisecode(), ordernumber, downIntegralAmount) );
