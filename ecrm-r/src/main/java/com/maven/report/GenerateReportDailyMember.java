...
//				ep.put("betTime", DateUtil.add(now, Calendar.DATE, -1));//投注日期为昨天的
				...
				for (BettingAllDay bad : bettingallemployee) { //计算用户投注内容
					...
					if("EN0044".equals(e.getEnterprisecode())){ //临时处理
						bad.setGameType(bad.getGameBigType());
						if(bad.getGameBigType().equals(GameBigType.DZ)){
							if(bad.getBetMoney()>1 && bad.getBetMoney()<1000000){
								ratio=new BigDecimal(0.01);
							}else if(bad.getBetMoney()>=1000000 && bad.getBetMoney()<3000000){
								ratio=new BigDecimal(0.012);
							}else{
								ratio=new BigDecimal(0.015);
							}
						}else if(bad.getGameBigType().equals(GameBigType.SX)){
							if(bad.getBetMoney()>1 && bad.getBetMoney()<1000000){
								ratio=new BigDecimal(0.008);
							}else if(bad.getBetMoney()>=1000000 && bad.getBetMoney()<3000000){
								ratio=new BigDecimal(0.009);
							}else{
								ratio=new BigDecimal(0.01);
							}
						}else if(bad.getGameBigType().equals(GameBigType.TY)){
								ratio=new BigDecimal(0.0045);
						}else if(bad.getGameBigType().equals(GameBigType.CP)){
							ratio=new BigDecimal(0.0045);
						}else if(bad.getGameBigType().equals(GameBigType.BY)){
