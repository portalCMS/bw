...
<script type="text/javascript">
	(function(){
		...
		var take_add = ${sessionScope.ERCM_USER_PSERSSION['MN006A'].menustatus == 1 };
		 ...
		 function permissionValidate(){
        	...
        	if(take_add) {
           		array.push({
                    ...
                    handler : function(){
                  	 openNewWindow('take_money_order','${ctx}/takeDepositRecord/TakeAdd','取款补单');
                    }
                });	
        	}
       		array.push({
                ...
                handler : function(){
                	...
                	searchForm.action = "${ctx}/takeDepositRecord/excelTakeApproveRecord";
                	searchForm.submit();
                	searchForm.action = "${ctx}/takeDepositRecord/findTakeApproveRecord";
                }
            });
       		return array;
		 }
	    ...
	    var Grid = BUI.Grid,
	      ...
	      columns = [
	        ...
	        { title: '取款人信息', width: '10%',  dataIndex: 'employeepaymentaccount', renderer:function(value,obj){
	        	return obj.employeepaymentname+"<br>"+obj.employeepaymentaccount+"<br>"+obj.bankname;
	        }},
	        ...
	        { title : '操作',		width: '10%',   renderer : function(value,obj){
	        	...
	        	if(order_audit&&obj.sign&&(!obj.handleemployee||obj.handleemployee=="${sessionScope.ERCM_USER_SESSION.loginaccount}")){
	        		html+= '<button onclick="showTakeApprove(\''+obj.sign+'\')" name="取款审批" class="button button-info"><span class="icon-check icon-white"></span>审批</button>';
	        	}
	        	html+= '<button onclick="showTakeInspect(\''+obj.ordernumber+'\',\''+obj.employeecode+'\',\''+obj.brandcode+'\')" name="稽查" class="button button-danger botton-margin"><span class="icon-eye-open icon-white"></span>稽查</button>';
	          	return html;
	        }}
	      ];

	var store = new Store({
	        url : '${ctx}/takeDepositRecord/findTakeApproveRecord',
	        autoLoad : false,
	        pageSize:15,
	        sortInfo : {
              field : 'orderdate',
