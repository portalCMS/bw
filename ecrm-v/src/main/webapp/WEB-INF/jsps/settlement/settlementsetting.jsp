		                ...
		                <div class="controls">
		                	...
		                	<input class="input-normal control-text" type="text" name="dividend" value="${user.dividend.setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue()*100 }" style="width:35px;height:18px;" data-rules="{required:true,number:true,min:${user.dividend*100 },max:${superior.dividend*100 }}"/> %
		                	 ...
		                	 max_share=${submaxbonus.get(max_share)*100 }
		                	  -->
		                	 <input class="input-normal control-text" type="text" name="dividend" value="<fmt:formatNumber type="number" value="${user.dividend * 100 }" pattern="0.00" maxFractionDigits="2"/>" style="width:50px;height:18px;" data-rules="{required:true,number:true,min:${submaxbonus.get(max_dividend)*100 },max:${superior.dividend*100 }}"/> % （最高<fmt:formatNumber type="number" value="${superior.dividend*100 }" pattern="0.00" maxFractionDigits="2"/> %）
		                </div>
		                <label class="control-label" style="margin-left: 50px">占&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;成:</label>
		                <div class="controls">
		                  <input class="input-normal control-text" type="text" name="share" value="<fmt:formatNumber type="number" value="${user.share * 100 }" pattern="0.00" maxFractionDigits="2"/>" style="width:50px;height:18px;" data-rules="{required:true,number:true,min:${submaxbonus.get(max_share)*100 },max:${superior.share*100 }}"/> % （最高<fmt:formatNumber type="number" value="${superior.share*100 }" pattern="0.00" maxFractionDigits="2"/> %）
		                </div>
	              	</div>
	              	</c:if>
	              	<hr/>	
		               		...
		               		<c:forEach var="game" varStatus="status" items="${games}">
			                	<div class="control-group" >
					                ...
					                <c:forEach items="${SystemCache.getInstance().getGameCategory(game.gametype)}" var="category">
							             ...
							             ${category.categoryname}：<input  class="input-normal control-text" type="text" name="${key}" value="<fmt:formatNumber type="number" value="${bonus.get(key).doubleValue()*100 }" pattern="0.00" maxFractionDigits="2"/>"  style="width:50px;height:18px;" data-rules="{required:true,number:true,min:${submaxbonus.get(key)*100},max:${maxbonus}}" /> % （最高<fmt:formatNumber type="number" value="${maxbonus}" pattern="0.00" maxFractionDigits="2"/> %）
							             </div>
							         </c:forEach>
					                </div>
				              	</div>
