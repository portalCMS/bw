...
@RequestMapping("/BET67SAFISH")
@Controller
public class Betting67SAFishController extends BaseController {
	
	private static LoggerManager log = LoggerManager.getLogger(
			Betting67SAFishController.class.getName(), OutputManager.LOG_BETTINGHQOGOG);
	...
	private BettingGameService<BettingBet67SAFish> bettingGameService;
	...
	public String list(HttpServletRequest request,Model model){
		...
		return "/gamerecord/bet67/safishgame";
	}
	...
	public Map<String,Object> data(HttpServletRequest request , HttpServletResponse response , HttpSession session){
		try {
			...
			List<BettingBet67SAFish> list = bettingGameService.takeRecord(object);
...
//			return super.formatPagaMap(list, count);
    ...
    public String exportExcel(HttpServletRequest request, HttpSession session, Model model){
        try {
            ...
            model.addAttribute("title", "SA-捕鱼游戏报表数据");
        } catch (Exception e) {
            log.Error(e.getMessage(), e);
        }
