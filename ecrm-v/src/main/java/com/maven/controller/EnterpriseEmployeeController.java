...
public class EnterpriseEmployeeController extends BaseController{
	...
	public String employeeData(Model model,HttpServletRequest request){
		...
		try {
			...
			model.addAttribute("listEnterpriseEmployeeType", LocalCache.getInstance().getAllEnterpriseEmployeeType());
		} catch (Exception e) {
...
//			System.out.println("Referer="+request.getHeader("Referer"));
			...
			if(users.size()==1){
				...
				if( !employee.getEmployeetypecode().equals(Type.代理.value) 
						&& !employee.getEmployeetypecode().equals(Type.大股东.value)
						&& !employee.getEmployeetypecode().equals(Type.股东.value) 
						&& !employee.getEmployeetypecode().equals(Type.总代理.value)
						&& !employee.getEmployeetypecode().equals(Type.信用代理.value)){
...
//		System.out.println(userbonus.doubleValue() > superbonus.doubleValue());
	...
	public String userAdd(Model model ,HttpSession session,HttpServletRequest request){
		...
		try {
			...
			model.addAttribute("enterprise",ee.getEmployeetypecode().equals(EnterpriseEmployeeType.Type.业主.value));
		...
		try {
			...
			if(__ee.getEmployeetypecode().equals(EnterpriseEmployeeType.Type.员工.value)){
				if(StringUtils.isBlank(registerEmployee.getEmployeetypecode())
						||registerEmployee.getEmployeetypecode().equals(EnterpriseEmployeeType.Type.业主.value)){
					...
					||registerEmployee.getEmployeetypecode().equals(EnterpriseEmployeeType.Type.大股东.value)
					||registerEmployee.getEmployeetypecode().equals(EnterpriseEmployeeType.Type.股东.value)
					||registerEmployee.getEmployeetypecode().equals(EnterpriseEmployeeType.Type.总代理.value)){
				if(StringUtils.isBlank(registerEmployee.getEmployeetypecode())
						||registerEmployee.getEmployeetypecode().equals(EnterpriseEmployeeType.Type.业主.value)
			...
			boolean isDirectly = __ee.getEmployeetypecode().equals(EnterpriseEmployeeType.Type.业主.value);
			//注册账号
...
//	         }
	...
	public Map<String,Object> deleteSelectEmployee(HttpServletRequest request,HttpSession session){
		try {
			...
			//只有员工类型与企业类型的用户具有权限
			if(loginEmployee.getEmployeetypecode().equals(Type.业主.value) 
	...
	public Map<String,Object> disableSelectEmployee(HttpServletRequest request,HttpSession session){
		try {
			EnterpriseEmployee loginEmployee = (EnterpriseEmployee) session.getAttribute(Constant.USER_SESSION);
			//只有员工类型与企业类型的用户具有权限
			if(loginEmployee.getEmployeetypecode().equals(Type.业主.value) || loginEmployee.getEmployeetypecode().equals(Type.员工.value)){
				String temp = request.getParameter("sign");
				String reason = request.getParameter("reason");
				String[] array = temp.split(",");
				boolean mark = this.decodeSign(array, session.getId());
	...
	private List<EmployeeGamecataloyBonus> bonusVerify(EnterpriseEmployee ee,Map<String,Object> object) throws Exception{
		...
		for (String key : object.keySet()) {
			if(key.indexOf("Game")>-1 || key.indexOf("BET67")>-1){
...
//				if(userbonus.compareTo(new BigDecimal("100"))==1){
	...
	public String topNameSearch(HttpServletRequest request, HttpSession session) {
		try {
			...
			if(Type.代理.value.equals(userType)||Type.大股东.value.equals(userType)
					||Type.股东.value.equals(userType)||Type.总代理.value.equals(userType)
					||Type.信用代理.value.equals(userType)) {
