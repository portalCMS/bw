...
public class EmployeeController extends BaseController {
	...
	private static LoggerManager log = LoggerManager.getLogger(EmployeeController.class.getName(),
			OutputManager.LOG_UER_EMPLOYEE);
	...
	public String login(HttpServletRequest request) {
		try {
			...
			AttrCheckout.checkout(object, false,
					new String[] { "loginaccount", "loginpassword", "loginip", "browserversion", "opratesystem" });
			...
			if (ee.getLoginpassword2() == null || ee.getLoginpassword2().equals("")) {
				...
				int status = enterpriseEmployeeService.tc_restPassword(AttrCheckout.checkout(object, true,
						new String[] { "loginpassword", "employeecode", "loginpassword2" }));
			}
			...
			Map<String, Object> result = AttrCheckout.checkout(BeanToMapUtil.convertBean(ee, true), true,
					...
					new String[] { "enterprisecode", "brandcode", "employeecode", "displayalias", "employeelevelcode",
							"employeelevelname", "employeetypecode", "employeetypename", "logindatetime",
							"lastlogintime", "parentemployeecode", "parentemployeeaccount", "fundpassword", "apiurl",
							"phoneno", "phonestatus", "email", "qq", "wechat", "alipay" });
			 ...
			 * // 获取游戏账号 EmployeeApiAccout __eaa =
			 * SystemCache.getInstance().getEmployeeGameAccount(ee.
			 * getEmployeecode(), Enum_GameType.环球.gametype); if(__eaa != null)
			 * { String userpwd = Base64.encode(("user=" +
			 * __eaa.getGameaccount() + "!password=" +
			 * __eaa.getGamepassword()).getBytes()); result.put("hyaccount",
			 * userpwd);// } H5百家乐独立版返回HY账号和密码
			 ****************************/
	...
	public String takeEmployeeLoginaccount2(HttpServletRequest request) {
		try {
			...
			Map<String, Object> result = AttrCheckout.checkout(BeanToMapUtil.convertBean(ee, true), true,
					new String[] { "loginaccount" },
					new String[] { "enterprisecode", "brandcode", "employeecode", "displayalias", "employeelevelcode",
							"employeelevelname", "employeelevelord", "employeetypecode", "employeetypename",
							"logindatetime", "lastlogintime", "parentemployeecode", "fundpassword", "phoneno", "email",
							"qq" });
			...
			/*
			 * List<BrandDomain> list = brandDomainService.select(augold); for
			 * (BrandDomain data : list) {
			 * if(data.getDomainlink().indexOf("678") > -1) { augold = data;
			 * break; } }
			 */
	...
	/**
	 * 获取用户信息-专用于GGPOKER收银台按钮直接登录 loginaccount参数必传
	 * 
	 * username=tony&date=2017-05-15T13:12:23Z&token=
	 * 17acf9535a0f4530847127c9a1f2a5cb
	 * 
	 * @param request
	 * @return
	 */
	...
	public String takeEmployeeLoginaccount(HttpServletRequest request) {
		try {
			...
			// token : MD5("123a456" + "myuser" + "654a321" +
			// "2015-05-15T13:12:23Z")
			...
			keydata = SystemCache.getInstance().getAPIEnterpriseKyes(apiAccout.getEnterprisecode(),
					Enum_GameType.GGPoker.gametype, keydata);
			...
			if (StringUtil.getString(ee.getEmployeelevelcode()).equals("")) {
				...
				ee.setEmployeelevelname(data.getEmployeeLevelName());
				ee.setEmployeelevelord(data.getOrd() + "");
				TakeMoneyOfDay = (data.getTakeMoneyOfDay() == null ? TakeMoneyOfDay
						: data.getTakeMoneyOfDay().doubleValue());
	...
	private void doRedbagActivity(EnterpriseEmployee employee, String loginip) throws Exception {
		...
		List<EnterpriseActivityCustomization> listEnterpriseActivityCustomization = enterpriseActivityCustomizationService
		...
		Map<String, Object> __activityAgument = enterpriseBrandActivityService
				.selectActivityAgument(activity.getEnterprisebrandactivitycode());
		...
		// 调用接口处理自动领取红包
		Map<String, Object> result = activityRedbagService.tc_redbag(employee.getEmployeecode(),
				activity.getEnterprisebrandactivitycode(), loginip);
		log.Error("=====================================================会员签到领红包结果：" + result + " ;"
				+ employee.getLoginaccount());
	...
	public String findUserFavourable(HttpServletRequest request) {
		try {
			...
			for (FavourableUser favourableUser : list) {
				...
				// Date1.after(Date2),当Date1大于Date2时，返回TRUE，当小于等于时，返回false
				// Date1.before(Date2)，当Date1小于Date2时，返回TRUE，当大于等于时，返回false
				if (!(currendate.after(favourableUser.getStarttime())
						&& currendate.before(favourableUser.getEndtime()))) {
					continue;
				}
	...
	public String takeEmployeeAccount(HttpServletRequest request) {
		try {
			...
			// Map<String, Game> BrandGameAll =
			// gameService.takeBrandGamesMap(ee.getBrandcode());

			Map<String, EmployeeApiAccout> map = SystemCache.getInstance()
					.getEmployeeAllGameAccount(ee.getEmployeecode());
			...
			if (map == null || map.size() == 0) {
				...
				// PlatformApiOutput key =
				// SystemCache.getInstance().getPlatformApiOutput(ee.getEnterprisecode());
				// return
				// Enum_MSG.成功.message(Encrypt.AESUNURLEncrypt(result.toString(),
				// key.getApikey1()));
				return Enum_MSG.成功.message(result);
			}
	...
	public String takeEmployee(HttpServletRequest request) {
		try {
			...
			Map<String, Object> result = AttrCheckout.checkout(BeanToMapUtil.convertBean(ee, true), true,
					new String[] { "loginaccount" },
					new String[] { "enterprisecode", "brandcode", "employeecode", "displayalias", "employeelevelcode",
							"employeelevelname", "employeelevelord", "employeetypecode", "parentemployeeaccount",
							"employeetypename", "logindatetime", "lastlogintime", "parentemployeecode", "fundpassword",
							"phoneno", "phonestatus", "email", "qq", "wechat", "alipay", "apiurl" });
	...
	public String UBankCards(HttpServletRequest request) {
		try {
			...
			List<EnterpriseEmployeeInformation> list = enterpriseEmployeeInformationService
					.select(AttrCheckout.checkout(ei, true, new String[] { "employeecode" },
							new String[] { "informationcode", "bankcode" }));
	...
	public String addUBankCard(HttpServletRequest request) {
		try {
			Map<String, Object> object = super.apiDecode(super.getRequestParamters(request));
			object = AttrCheckout.checkout(object, true,
					new String[] { "enterprisecode", "bankcode", "employeecode", "paymentaccount", "accountname",
							"openningbank", "fundpassword" },
			...
			// 户名不得包含数字
			if ((!bankcode.equals("B019") && !bankcode.equals("B020"))
					&& com.maven.util.StringUtils.isNumeric(ss.getAccountname())) {
				return Enum_MSG.逻辑事物异常.message(Enum_MSG.开户名称不得包含数字.desc);
			}

			// 卡bin方式校验银行卡格式规范
			if ((!bankcode.equals("B019") && !bankcode.equals("B020"))
					&& BankCheckUtil.checkBankCard(ss.getPaymentaccount()) == null) {
			...
			/************************************
			 * 以下信息请注意参考处理：姓名王勇，电话号是18656588180， 开户地址：哈尔滨，经核实这个人是专业做恶意举报的一位人士，
			 * 经常提现消费了去举报要求退款，常使用的银行卡有 光大银行卡号：6226620306419987，
			 * 农业银行卡号：6228480178500953975， 中国银行卡号：6217855300024014973
			...
			if (ss.getPaymentaccount().equals("6226620306419987")
					|| ss.getPaymentaccount().equals("6228480178500953975")
					|| ss.getPaymentaccount().equals("6217855300024014973")
					|| ss.getPaymentaccount().equals("6217001500001379074")
					|| ss.getPaymentaccount().equals("6222031206000181727")
					|| ss.getPaymentaccount().equals("6226097812597715")
					|| ss.getPaymentaccount().equals("6214857556077446")
					|| ss.getPaymentaccount().equals("6217256300009023887")) {

				return Enum_MSG.逻辑事物异常.message("此银行卡号存在异常，请您核实之后与客服联系咨询");
			}
			if (ss.getPhonenumber() != null && (ss.getPhonenumber().equals("18656588180")
					|| ss.getPhonenumber().equals("13424261271") || ss.getPhonenumber().equals("18513139448")
					|| ss.getPhonenumber().equals("18256560131") || ss.getPhonenumber().equals("15798950232"))) {
			...
			userLogsService.addActivityBetRecord(new UserLogs(ee.getEnterprisecode(), ee.getEmployeecode(),
					ee.getLoginaccount(), UserLogs.Enum_operatype.银行卡业务, "绑定了新的银行卡:" + ss.getBankname(), null, null));
	...
	public String editUserBankCard(HttpServletRequest request) {
		try {
			...
			Map<String, Object> object = super.apiDecode(super.getRequestParamters(request));
			object = AttrCheckout.checkout(object, true,
					new String[] { "enterprisecode", "employeecode", "informationcode", "fundpassword" },
					new String[] { "phonenumber", "qq", "skype", "email", "paymentaccount", "accountname",
							"openningbank", "bankcode" });
			EnterpriseEmployee ee = enterpriseEmployeeService.takeEnterpriseEmployee(object);
			...
			userLogsService.addActivityBetRecord(new UserLogs(ee.getEnterprisecode(), ee.getEmployeecode(),
					ee.getLoginaccount(), UserLogs.Enum_operatype.银行卡业务, "更改了银行卡:" + ss.getBankname(), null, null));
	...
	public String DefaultUBankCard(HttpServletRequest request) {
		try {
			Map<String, Object> object = super.apiDecode(super.getRequestParamters(request));
			object = AttrCheckout.checkout(object, true,
					new String[] { "enterprisecode", "employeecode", "informationcode" });
			EnterpriseEmployee ee = enterpriseEmployeeService.takeEnterpriseEmployee(object);
			...
			if (!__eei.getStatus().equals(Enum_status.解锁.value)
					&& ee.getEmployeetypecode().equals(EnterpriseEmployeeType.Type.会员.value)) {
	...
	public String delUBankCard(HttpServletRequest request) {
		try {
			...
			int status = enterpriseEmployeeInformationService.tc_deleteEmployeeBankCard(ee.getEmployeecode(),
					informationcode);

			userLogsService.addActivityBetRecord(new UserLogs(ee.getEnterprisecode(), ee.getEmployeecode(),
					ee.getLoginaccount(), UserLogs.Enum_operatype.银行卡业务, "删除了的银行卡:" + __eei.getBankname(), null, null));
	...
	public String updateFunpassword(HttpServletRequest request) {
		try {
			...
			int status = enterpriseEmployeeService.tc_restPassword(AttrCheckout.checkout(object, true,
					new String[] { "fundpassword" }, new String[] { "employeecode" }));

			userLogsService.addActivityBetRecord(new UserLogs(ee.getEnterprisecode(), ee.getEmployeecode(),
					ee.getLoginaccount(), UserLogs.Enum_operatype.用户资料业务, "修改了资金密码", null, null));
	...
	public static void main(String[] args) {
		// System.out.println(Encrypt.MD5("4ujsx2qjpqg" + "egg2230" + new
		// StringBuffer("4ujsx2qjpqg").reverse().toString() + "xffdfdfdfd"));
		Object val = -1;
	...
	public String updateLoginPassword(HttpServletRequest request) {
		try {
			Map<String, Object> object = super.apiDecode(super.getRequestParamters(request));
			AttrCheckout.checkout(object, false,
					new String[] { "employeecode", "oldloginpassword", "newloginpassword" });
			EnterpriseEmployee ee = super.menberAuthentication(
					enterpriseEmployeeService.takeEnterpriseEmployee(object));
			if (ee.getLoginpassword() != null
					&& !ee.getLoginpassword().equals(Encrypt.MD5(String.valueOf(object.get("oldloginpassword"))))) {
			...
			userLogsService.addActivityBetRecord(new UserLogs(ee.getEnterprisecode(), ee.getEmployeecode(),
					ee.getLoginaccount(), UserLogs.Enum_operatype.用户资料业务, "修改了登录密码", null, null));
	...
	public String resetLoginPassword(HttpServletRequest request) {
		try {
			...
			userLogsService.addActivityBetRecord(new UserLogs(ee.getEnterprisecode(), ee.getEmployeecode(),
					ee.getLoginaccount(), UserLogs.Enum_operatype.用户资料业务, "重置了登录密码", null, null));
	...
	public String getVerifycode(HttpSession session, HttpServletRequest request) {
		try {
			...
			// 120秒限时，禁止重复提交
			// RepeatRequestIntercept.isIntercept(phoneno,
			// request.getRequestURI().replaceAll("/+", "/"), 5 * 60 *1000);

			// RepeatRequestIntercept.isIntercept("ip"+ip,
			// request.getRequestURI().replaceAll("/+", "/"), 5 * 60 *1000);
			...
			// if(val instanceof Integer && Integer.valueOf(val.toString()) ==
			// -1) {
			// falg = true;
			// }
			// }
			...
			// return
			// Enum_MSG.成功.message(RandomStringNum.createRandomString(4));
			return Enum_MSG.成功.message(smscode);
	...
	public String saveEmployee(HttpSession session, HttpServletRequest request) {
		try {
			Map<String, Object> object = super.apiDecode(super.getRequestParamters(request));
			AttrCheckout.checkout(object, false,
					new String[] { "enterprisecode", "brandcode", "domain", "loginaccount", "loginpassword" });
			...
			List<EnterpriseEmployee> isExits = enterpriseEmployeeService
					.queryEmployeeIsExist(enterpriseEmployee.getLoginaccount());
			...
			if (object.get("phoneno") != null && object.get("phoneno").toString().length() == 11) {
				...
				int count = enterpriseEmployeeService.queryLoginNamePhoneExists(object.get("phoneno").toString());
				if (count > 0) {
			...
			// 获取推广用户的信息
			EnterpriseEmployee employee = enterpriseEmployeeService
					.takeEmployeeByCode(registerSiteAgument.getEmployeecode());

			// 员工的上级编码为当前登录用户编码
			enterpriseEmployee.setParentemployeecode(employee.getEmployeecode());
			// 登陆密码加密
			enterpriseEmployee.setLoginpassword2(
					APIServiceUtil.encrypt(enterpriseEmployee.getLoginpassword(), enterpriseEmployee));// 对原始密码进行加密
			...
			if (displayalias != null && !displayalias.equals("null") && displayalias.trim().length() >= 2) {
				...
				displayalias = enterpriseEmployee.getLoginaccount().length() > 8
						? enterpriseEmployee.getLoginaccount().substring(0, 8)
						: enterpriseEmployee.getLoginaccount().substring(0,
								enterpriseEmployee.getLoginaccount().length());
			}
			...
			// 注册账号
			boolean isDirectly = employee.getEmployeetypecode().equals(EnterpriseEmployeeType.Type.业主.value);
			...
			/*************************
			 * 处理默认优惠组业务**************************暂时先取消，之前来博有这个需求 Favourable
			 * condition = new Favourable();
			 * condition.setEnterprisecode(employee.getEnterprisecode());
			 * condition.setIsdeault(Favourable.Enum_isdeault.默认.value);
			 * condition.setStatus(Favourable.Enum_status.启用.value); List
			 * <Favourable> templist =
			 * favourableService.select(condition);//查询此人当前拥有的优惠组
			 * 
			 * Date currendate = new Date(); for (Favourable favourable :
			 * templist) { // 活动是否进行中
			 * //Date1.after(Date2),当Date1大于Date2时，返回TRUE，当小于等于时，返回false
			 * //Date1.before(Date2)，当Date1小于Date2时，返回TRUE，当大于等于时，返回false if(
			 * !(currendate.after(favourable.getStarttime()) &&
			 * currendate.before(favourable.getEndtime()) )){ continue ; }
			 * FavourableUser user = new
			 * FavourableUser(UUID.randomUUID().toString(), favourable.getLsh(),
			 * employee.getEnterprisecode(), employee.getEmployeecode());
			 * favourableUserService.addActivityBetRecord(user); } 处理默认优惠组业务
			 **************************/
			...
			if (enterpriseEmployee.getEnterprisecode().equals("EN003K")) {
				...
				List<Game> games = gameService.takeBrandGames(enterpriseEmployee.getBrandcode(),
				...
				if (isOpenGame(enterpriseEmployee.getBrandcode(), Enum_GameType.GGPoker.gametype, games) == null) {
					...
					EmployeeApiAccout eea = SystemCache.getInstance().getEmployeeGameAccount(
							enterpriseEmployee.getEmployeecode(), Enum_GameType.GGPoker.gametype);
					if (eea == null) {
						JSONObject jsonObject = JSONObject
								.fromObject(api.create(Enum_GameType.GGPoker.gametype, enterpriseEmployee));
						eea = SystemCache.getInstance().getEmployeeGameAccount(enterpriseEmployee.getEmployeecode(),
								Enum_GameType.GGPoker.gametype);
						log.Error("===金蛋会员注册===创建GG扑克游戏账号结果：" + jsonObject);
					}
					/**
					 * eea = SystemCache.getInstance().getEmployeeGameAccount(
					 * enterpriseEmployee.getEmployeecode(),
					 * Enum_GameType.IDN.gametype); if(eea == null) { JSONObject
					 * jsonObject = JSONObject.fromObject(
					 * api.create(Enum_GameType.IDN.gametype,
					 * enterpriseEmployee) ); eea =
					 * SystemCache.getInstance().getEmployeeGameAccount(
					 * enterpriseEmployee.getEmployeecode(),
					 * Enum_GameType.IDN.gametype);
			...
			if (StringUtil.getString(object.get("H5BJL")).toUpperCase().equals("Y")) {
				APIServiceNew api = new APIServiceNew(enterpriseEmployee.getEnterprisecode());
				EmployeeApiAccout eea = SystemCache.getInstance()
						.getEmployeeGameAccount(enterpriseEmployee.getEmployeecode(), Enum_GameType.环球.gametype);
				if (eea == null) {
					JSONObject jsonObject = JSONObject
							.fromObject(api.create(Enum_GameType.环球.gametype, enterpriseEmployee));
					eea = SystemCache.getInstance().getEmployeeGameAccount(enterpriseEmployee.getEmployeecode(),
			 ...
			 * APIServiceNew api = new
			 * APIServiceNew(enterpriseEmployee.getEnterprisecode());
			 * EmployeeApiAccout eea =
			 * SystemCache.getInstance().getEmployeeGameAccount(
			 * enterpriseEmployee.getEmployeecode(), Enum_GameType.棋牌.gametype);
			 * if(eea == null) { JSONObject jsonObject = JSONObject.fromObject(
			 * api.create(Enum_GameType.棋牌.gametype, enterpriseEmployee) ); eea
			 * = SystemCache.getInstance().getEmployeeGameAccount(
			 * enterpriseEmployee.getEmployeecode(), Enum_GameType.棋牌.gametype);
			...
			userLogsService.addActivityBetRecord(
					new UserLogs(enterpriseEmployee.getEnterprisecode(), enterpriseEmployee.getEmployeecode(),
							enterpriseEmployee.getLoginaccount(), UserLogs.Enum_operatype.用户资料业务, "注册会员", null, null));
	...
	public String h5Login(HttpServletRequest request) {
		try {
			...
			return Enum_MSG.成功.message(
					AttrCheckout.checkout(BeanToMapUtil.convertBean(ee, true), true, new String[] { "loginaccount" },
							new String[] { "enterprisecode", "brandcode", "employeecode", "employeelevelcode",
									"employeelevelname", "employeetypecode", "employeetypename", "logindatetime",
									"lastlogintime", "parentemployeecode", "fundpassword" }));
		} catch (ArgumentValidationException e) {
			log.Error(e.getMessage(), e);
			return Enum_MSG.参数错误.message(e.getMessage());
		} catch (LogicTransactionRollBackException e) {
	 ...
	 * 修改用户资料
	 * 
	 * qq=qq号码 email=邮箱号码 phoneno=手机号码
	 * 
	 * @return
	 */
		...
		try {
			...
			if (object.containsKey("email")) {
				if (String.valueOf(object.get("email")).length() > 0
						&& !com.maven.util.StringUtils.checkEmail(String.valueOf(object.get("email")))) {
			...
			if (object.containsKey("qq")) {
				if (String.valueOf(object.get("qq")).length() > 0
						&& !com.maven.util.StringUtils.isNumber(String.valueOf(object.get("qq")))) {
			...
			if (prams.size() > 1) {
				...
				if (status == 1) {
					...
					userLogsService.addActivityBetRecord(new UserLogs(ee.getEnterprisecode(), ee.getEmployeecode(),
							ee.getLoginaccount(), UserLogs.Enum_operatype.用户资料业务, "修改了邮箱、qq、手机号码等", null, null));
