...
<head>
	...
	<%
		...
		String url_for_post = url_for_get.substring(0,url_for_get.indexOf("?")==-1?url_for_get.length():url_for_get.indexOf("?"));
		String url_params = "";
		if(url_for_get.indexOf("?")!=-1){
			url_params = url_for_get.substring(url_for_get.indexOf("?")+1);
		}
		...
		if(url_for_get.startsWith("https://pay.islpay.com/gateway") ) {
			url_for_post = "https://pay.islpay.com/gateway?input_charset=UTF-8";
		}
    ...
    <%
    	...
    	for(String s : url_params_array){
    		...
    		if(keyvalue.length > 0){
    			int index = s.indexOf("=");
    			String name = index==-1?"":s.substring(0, index);
