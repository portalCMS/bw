...
public class TAGGame {	
	...
	private static Logger logger = LogManager.getLogger(TAGGame.class.getName());
	/**
	 * 获取TAG游戏数据列表
	 * @param agent 代理帐号
	 * @param dir 数据文件目录
...
//					System.out.println(agent+"===========================p2getTAGData-2");
			...
			logger.error(ex);
			ex.printStackTrace();
			LogUtil.addListLog(LogUtil.HANDLE_TAG, dir, ex.getMessage(), agent, Enum_flag.异常.value);
			return null;
		}
	}
	...
	public static List<Map<String, Object>> getTAGData(String agent, FtpUtils ftp,List<String> ls) {
		try {
			...
			logger.error(ex);
			ex.printStackTrace();
			LogUtil.addListLog(LogUtil.HANDLE_TAG, agent, ex.getMessage(), agent, Enum_flag.异常.value);
			return null;
		}
	}
