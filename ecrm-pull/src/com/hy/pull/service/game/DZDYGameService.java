...
import com.hy.pull.common.util.game.DZDYGame;
import com.hy.pull.mapper.DataHandleLogsMapper;
import com.hy.pull.mapper.DataHandleMapper;
import com.hy.pull.mapper.DzpkGameinfoMapper;
 ...
 */
@Service
public class DZDYGameService extends BaseService{
	...
	@Autowired
	private DzpkGameinfoMapper dzpkGameinfoMapper;
	...
	public Integer pullData(Map<String, Object> entity) throws Exception {
		...
		/**********************获取上次拉取的最大值
		Map<String, Object> dataHandle = dataHandleMapper.selectByPrimaryKey(this.HANDLE_DZDY);
		...
		***********************/
		...
		if(!entity.containsKey("GAME_ID")){//判断是否存在游戏编号，没则默认初始化为德州扑克游戏
		...
		if(size > 0){
			...
			*****************************/
			...
			for(int i = 0; i < size; i++){
				...
				channelId = entity.get("PROXY_NAME").toString();
				md5Key = entity.get("PROXY_MD5_KEY").toString();
				code = entity.get("PROXY_CODE") == null ? channelId : entity.get("PROXY_CODE").toString();
				data = DZDYGame.getData(apiUrl, channelId, md5Key, stardate,enddate,code);//获取拉取数据列表;
				if(data != null){
					len = data.size();
					count += len;
					if(len > 0){//如果有数据就入库
						dzpkGameinfoMapper.batchInsert(data);//批量入库
