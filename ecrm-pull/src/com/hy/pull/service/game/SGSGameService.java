 ...
 */
@Service
public class SGSGameService extends BaseService {
	
	Logger logger = LogManager.getLogger(SGSGameService.class.getName());  
	
	@Autowired
	private ApiSgsGameinfoMapper apiSgsGameinfoMapper;
	...
	public Integer pullData(Map<String, Object> entity) throws Exception {
		...
		/**********************获取上次拉取的最大值***********************/
		Map<String, Object> dataHandle = dataHandleMapper.selectByPrimaryKey(LogUtil.HANDLE_SGS);
		...
		if(!entity.containsKey("GAME_ID")){//判断是否存在游戏编号，没则默认初始化为东方游戏
			entity.put("GAME_ID", "17");//设置所属游戏编号
		}
		//获取东方游戏的所有代理密钥列表
		...
		if(size > 0){
			...
			for(int i = 0; i < size; i++){
				...
				client_id = entity.get("PROXY_NAME").toString();
				client_secret = entity.get("PROXY_KEY1").toString();
				code = entity.get("PROXY_CODE").toString();
				map.put("Platformflag", code);
				
				
				data = SGSGame.getSGSData(API_URL, client_id, client_secret, code, 
				...
				if(data != null){
					...
					//dataHandleLogsMapper.insert(LogUtil.saveLog(LogUtil.HANDLE_SGS, null, len+"个", client_id, Enum_flag.正常.value));
					...
					dataHandleLogsMapper.insert(LogUtil.saveLog(LogUtil.HANDLE_SGS, null, "返回NULL数据", client_id, Enum_flag.异常.value));
