 ...
 */
@Service
public class AOIGameService extends BaseService {
	
	Logger logger = LogManager.getLogger(AOIGameService.class.getName());  
	
	@Autowired
	private AoiGameinfoMapper aoiGameinfoMapper;
	...
	public Integer pullData(Map<String, Object> entity) throws Exception {
		...
		/**********************获取上次拉取的最大值***********************/
		Map<String, Object> dataHandle = dataHandleMapper.selectByPrimaryKey(LogUtil.HANDLE_AOI);
		dataHandle.put("lastnum", "0");
		String updatetime = dataHandle.get("updatetime").toString();//agent=10;
		...
		if(size > 0){
			...
			for(int i = 0; i < size; i++){
				...
				if(MAX_VALUE == null){//如果为空则获取数据库最大值
					map.put("Platformflag", agent);
					vendorid = aoiGameinfoMapper.max(map);					
					if(vendorid == null){
						//map.put("Platformflag", null);
						vendorid = "0";
					}
				}
				data = AOIGame.getAOIData(apiUrl, agent, vendorid, userKey,code);//获取拉取数据列表;
				if(data != null){
					len = data.size();
					count += len;//累计执行结果
					if(len > 0){//如果有数据就入库						
						aoiGameinfoMapper.batchInsert(data);//批量入库
						
						/**************取这批数据的最大值ibc_vendorid****************/
						long aoi_VendorId = 0;
						for (Map<String, Object> map2 : data) {
							if(Long.valueOf(map2.get("aoi_VendorId").toString()) > aoi_VendorId) {
								aoi_VendorId = Long.valueOf(map2.get("aoi_VendorId").toString());
							}
						}
						...
						mapMaxValue.put(code, aoi_VendorId+"");//更新
					}
					
					//dataHandleLogsMapper.insert(LogUtil.saveLog(LogUtil.HANDLE_AOI, null, count+"成功", agent, Enum_flag.正常.value));
					
				} else {
					//为null表示出现异常
					flag = false;
					
					dataHandleLogsMapper.insert(LogUtil.saveLog(LogUtil.HANDLE_AOI, null, "返回NULL数据", agent, Enum_flag.异常.value));
