 ...
 */
@Service
public class NHQGameService extends BaseService {
	
	Logger logger = LogManager.getLogger(NHQGameService.class.getName());  
	
	@Autowired
	private TbProxyKeyMapper tbProxyKeyMapper;
	
	@Autowired
	private NhqGameinfoMapper nhqGameinfoMapper;
	...
	public Integer pullData(Map<String, Object> entity) throws Exception {
		...
		/**********************获取上次拉取的最大值***********************/
		Map<String, Object> dataHandle = dataHandleMapper.selectByPrimaryKey(LogUtil.HANDLE_NHQ);
		...
		if(size > 0){
			...
			String agentname = null;//代理名称
			String agentpwd = null;//代理密码
			...
			if(entity.get("USER_NAME") != null){//帐号
				username = entity.get("USER_NAME").toString();
			}
			...
			for(int i = 0; i < size; i++){
				...
				agentname = entity.get("PROXY_NAME").toString();
				agentpwd = entity.get("PROXY_MD5_KEY").toString();
				deskey = entity.get("PROXY_KEY1").toString();
				md5key = entity.get("PROXY_KEY2").toString();
				code = entity.get("PROXY_CODE") == null ? agentname : entity.get("PROXY_CODE").toString();
				map.put("Platformflag", agentname);
				
				if(stardate == null){//如果为空则获取数据库最大值
					stardate = nhqGameinfoMapper.max(map);
				}
				data = NHQGame.getNHQData(apiUrl, username, agentname,agentpwd,deskey,md5key,stardate,enddate,code);//获取拉取数据列表;
				if(data != null){
					len = data.size();
					count += len;
					if(len > 0){//如果有数据就入库
					...
					//dataHandleLogsMapper.insert(LogUtil.saveLog(LogUtil.HANDLE_NHQ, null, len+"个", agentname, Enum_flag.正常.value));
					...
					dataHandleLogsMapper.insert(LogUtil.saveLog(LogUtil.HANDLE_NHQ, null, "返回NULL数据", agentname, Enum_flag.异常.value));
