 ...
 */
@Service
public class TAGGameService2 extends BaseService {

	
	Logger logger = LogManager.getLogger(TAGGameService2.class.getName());  
	...
	public int doInsertData() {
		...
		if(list != null && list.size() > 0) {
			...
			for (int i = 0; i < size1; i++) {
				...
				data = list.get(i);
				if( data.get("platformtype").toString().equals("YOPLAY")) {
					continue;
				}
		...
		try {
			...
			while (iter.hasNext()) {
				...
				if ("HSR".equals(dataType)) {//捕鱼王
					...
					//非投注记录
					if(itemEle.attributeValue("billNo") == null) {
						continue;
					}
...
//					System.out.println("XML文件billNo为空:"+data);
				}
			}
			int a = 0;
			if(listInsert.size() > 0) {
				a = tagGameinfoMapper.batchInsert(listInsert);
			}
	...
	public static void main(String[] args) {
		...
		try {
			...
			currendate = calendar.getTime();
			String lastDate = DateUtil.parse(currendate, "yyyyMMdd");
			System.out.println(lastDate+"1200");
