...
import com.hy.pull.common.util.game.IDNGame2;
import com.hy.pull.common.util.game.SGSGame;
import com.hy.pull.mapper.AoiGameinfoMapper;
import com.hy.pull.mapper.ApiIdn2GameinfoMapper;
 ...
 */
@Service
public class IDNGameService2 extends BaseService {
	
	Logger logger = LogManager.getLogger(IDNGameService2.class.getName());  
	
//	@Autowired
//	private ApiIdnGameinfoMapper apiIdnGameinfoMapper;
	@Autowired
	private ApiIdn2GameinfoMapper apiIdn2GameinfoMapper;
	...
	public Integer pullData(Map<String, Object> entity) throws Exception {
		...
		if(size > 0){
			...
			stardate = updatetime;//后退一分钟
			enddate = DateUtil.parse(DateUtil.add(DateUtil.parse(stardate, "yyyy-MM-dd"), Calendar.DATE, 1), "yyyy-MM-dd");//日期加1
			if(DateUtil.parse(enddate, "yyyy-MM-dd").getTime() > new Date().getTime()) {
			...
			for(int i = 0; i < size; i++){
				...
				data = IDNGame2.getDataList(API_URL, secret_key, stardate);
				if(data != null){
					len = data.size();
					count += len;//累计执行结果
					if(len > 0){//如果有数据就入库						
						apiIdn2GameinfoMapper.batchInsert(data);//批量入库
