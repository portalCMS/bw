...
import com.hy.pull.common.util.game.WIN88Game;
import com.hy.pull.mapper.ApiWin88GameinfoMapper;
 ...
 */
@Service
public class WIN88GameService extends BaseService {

	
	Logger logger = LogManager.getLogger(WIN88GameService.class.getName());  
	
	@Autowired
	private ApiWin88GameinfoMapper ptGameinfoMapper;
	...
	public Integer pullData(Map<String, Object> entity) throws Exception {
		...
		/**********************获取上次拉取的最大值***********************/
		Map<String, Object> dataHandle = dataHandleMapper.selectByPrimaryKey(LogUtil.HANDLE_WIN88);
		...
		if(!entity.containsKey("GAME_ID")){//判断是否存在游戏编号，没则默认初始化为PT游戏
			entity.put("GAME_ID", "26");//设置所属游戏编号				
		}
		//获取PT游戏的所有代理密钥列表
		...
		if(size > 0){
			...
			for(int i = 0; i < size; i++){
				...
				info.put("keyStore", entity.get("PROXY_KEY1").toString());//秘钥
...
//					stardate = ptGameinfoMapper.max(map);
				...
				data = WIN88Game.getPTData(apiUrl, agent,stardate, enddate,info,code);//获取拉取数据列表;
				if(data != null){
					len = data.size();
					count += len;//累计执行结果
					if(len > 0){//如果有数据就入库
					...
					//dataHandleLogsMapper.insert(LogUtil.saveLog(LogUtil.HANDLE_WIN88, null, len+"个", agent, Enum_flag.正常.value));
					...
					dataHandleLogsMapper.insert(LogUtil.saveLog(LogUtil.HANDLE_WIN88, null, "返回NULL数据", agent, Enum_flag.异常.value));
