...
<body class="sidebar-default header-fixed">
						...
						<div class="panel-body">
							...
							<div id="no-more-tables" style="margin-top:5px;">
								<display:table id="row" name="list" class="table table-bordered table-striped table-condensed table-advanced cf" partialList="true" size="resultSize"
									pagesize="${pageSize}" requestURI="${pageContext.request.contextPath}/manager/game/list.do">
									<display:column title="<input type='checkbox'  class='checkall' />" style="text-align:center" headerClass="ct">
										<input type="checkbox" value="${row.GAME_ID}" name="answer">
									</display:column>
									<display:column property="GAME_ID" title="编号" scope="编号" />
									<display:column property="GAME_NAME" title="名称" scope="名称" />
						...
						<div class="row">
							<div class="col-md-12">
								<div class="form-group">
									<label for="GAME_NAME" class="control-label">名称</label> <input id="GAME_NAME" name="GAME_NAME" type="text" placeholder="" class="form-control" emsg="请输入名称" />
								</div>
							</div>
						</div>
					</form>
	...
	<div id="editPopup" tabindex="-1" data-backdrop="static" data-keyboard="false" class="modal fade" style="z-index: 99999">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" data-dismiss="modal" aria-hidden="true" class="close">&times;</button>
					<h4 class="modal-title">编辑游戏</h4>
				</div>
				<div class="modal-body">
					<form id="editForm" name="editForm">
						<input type="hidden" id="editId" name="GAME_ID" />
						<div class="row">
							<div class="col-md-12">
								<div class="form-group">
									<label for="editName" class="control-label">名称</label> <input id="editName" name="GAME_NAME" type="text" placeholder="" class="form-control" />
								</div>
							</div>
						</div>
					</form>
	...
	<script type="text/javascript">
		...
		var delAction = "${pageContext.request.contextPath}/manager/game/delete.do";
		var addAction = "${pageContext.request.contextPath}/manager/game/add.do";
		var editAction = "${pageContext.request.contextPath}/manager/game/update.do";
		//删除全部选中项
		function del() {
			delAll(delAction);
		}
		...
		function showEdit() {
			...
			if (len == 0) {
				...
				var admin = getJsonObject("json","GAME_ID",value);
				
				$("#editName").val(admin["GAME_NAME"]);
				$("#editId").val(admin["GAME_ID"]);
